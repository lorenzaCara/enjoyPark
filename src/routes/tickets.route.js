...
      // Validazione della data: validFor deve essere nel futuro (inizio del giorno in UTC)
      const validForStartOfDayUTC = new Date(validFor + "T00:00:00.000Z");
...
      const validForEndOfDayUTC = new Date(validFor + "T23:59:59.999Z");

      const nowUTC = new Date();
      const isExpired = nowUTC.getTime() > validForEndOfDayUTC.getTime();
...
          qrCode: qrCodeImage,
          rawCode,
          validFor: validForEndOfDayUTC, // Salviamo la fine del giorno in UTC
...
      });
      if (!ticket) return res.status(404).json({ error: "Ticket not found" });
      // Questa condizione è molto restrittiva: solo l'utente proprietario può aggiornare.
      // Lo staff middleware dovrebbe già gestire i permessi di update, quindi
      // potresti voler rimuovere questa riga se lo staff deve poter aggiornare ticket non propri.
      if (ticket.userId !== req.user.id)
        return res.status(403).json({ error: "Permission denied" });
...
        updatedValidFor = new Date(validFor + "T23:59:59.999Z");
...
      } else {
        // Gestisci altri casi, ad esempio se validFor è null/undefined e non deve essere aggiornato
        // o se è un formato non riconosciuto. Per ora, lo lasciamo invariato.
        // Potresti voler aggiungere una validazione più stretta qui o nel middleware.
      }

      const now = new Date();
      // Se updatedValidFor non è un oggetto Date valido qui, il confronto sotto darà problemi.
      // È fondamentale che updatedValidFor sia un Date valido.
      const computedStatus =
        status ||
        (updatedValidFor && now.getTime() > updatedValidFor.getTime() ? TicketStatus.EXPIRED : TicketStatus.ACTIVE);

      const updatedTicket = await prisma.ticket.update({
        where: { id: ticketId },
        data: {
          ticketTypeId,
          validFor: updatedValidFor, // <--- Modificato qui
          discountId: discountId || null,
          paymentMethod: paymentMethod || null,
          status: computedStatus,
        },
...
        return res
          .status(400)
          .json({ error: `Ticket not valid, status: ${ticket.status}` });
      }

      const todayUTC = new Date();
      todayUTC.setUTCHours(0, 0, 0, 0);

      const validForDateFromDB = new Date(ticket.validFor);
      const validForDayUTC = new Date(
        validForDateFromDB.toISOString().split("T")[0] + "T00:00:00.000Z"
      );

      if (validForDayUTC.getTime() !== todayUTC.getTime()) {
        return res
          .status(400)
          .json({
